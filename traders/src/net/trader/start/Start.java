...
public class Start extends JFrame {
	...
	private void initComponents() {
		...
		add(getObjTotalAmoutLabel(), new Constraints(new Leading(492, 12, 12), new Leading(455, 12, 12)));
		add(getObjLabelTotalAmountLabel(), new Constraints(new Leading(602, 102, 12, 12), new Leading(453, 24, 12, 12)));
		...
		add(getObjARtradersLabel(), new Constraints(new Bilateral(4, 0, 10), new Leading(10, 572, 10, 10)));
		setSize(719, 589);
	}
	...
	private JLabel getObjLabelTotalAmountLabel() {
		if (objLabelTotalAmountLabel == null) {
			...
			objLabelTotalAmountLabel.setText("95.69");
		}
	...
	private JTextField getObjTaxTextField() {
		if (objTaxTextField == null) {
			objTaxTextField = new JFormattedTextField(new Double(5.0));
		}
...
//			jInvocieSearchTextField.setEditable(false);
	...
	private JLabel getJsearchLabel() {
		if (jSearchLabel == null) {
			...
			jSearchLabel.setBorder(BorderFactory.createTitledBorder(null, "Search", TitledBorder.CENTER, TitledBorder.TOP, null, null));
		}
		return jSearchLabel;
	}
	...
	private JLabel getCreateLabel() {
		if (jCreateLabel == null) {
			...
			jCreateLabel.setBorder(BorderFactory.createTitledBorder(null, "Create", TitledBorder.CENTER, TitledBorder.DEFAULT_POSITION, null, null));
		}
		return jCreateLabel;
	}
	...
	private JScrollPane getObjMaterialdetailScrolPance() {
		if (objMaterialdetailsTable == null) {
			...
			objMaterialdetailsTable.setViewportView(getObjMaterialdetailsTable());
		}
	...
	private JTable getObjMaterialdetailsTable() {
		if (objMeterialdetailsTable == null) {
			...
			objMeterialdetailsTable.setModel(objDefaultTableModel=new DefaultTableModel(new Object[][][] {}, new String[] { "Material Description", "Quantity","Unit Rate","Amount"}) {
				private static final long serialVersionUID = 1L;
			...
			objMeterialdetailsTable.getColumnModel().getColumn(0).setPreferredWidth(700);
			objMeterialdetailsTable.getColumnModel().getColumn(1).setPreferredWidth(100);
			objMeterialdetailsTable.getColumnModel().getColumn(2).setPreferredWidth(100);
			objMeterialdetailsTable.getColumnModel().getColumn(3).setPreferredWidth(100);
			objMeterialdetailsTable.getColumnModel().getColumn(3).setCellRenderer(new DecimalFormatRenderer() );
	...
	private JLabel getObjARtradersLabel() {
		if (objARtradersLabel == null) {
			...
			objARtradersLabel.setBorder(BorderFactory.createTitledBorder(new LineBorder(Color.black, 1, true), "A.R.TRADERS", TitledBorder.CENTER,
					TitledBorder.TOP, new Font("Dialog", Font.BOLD, 20), Color.blue));
		}
	...
	private void objAddbuttonActionActionPerformed(ActionEvent event) {
		...
		for(int i=0;i<objDefaultTableModel.getRowCount();i++){
			if(objMeterialdetailsTable.getModel().getValueAt(i,1).toString().length()==0){
				statu=false;
				break;
			}
		}
	...
	private void jSearchButtonActionActionPerformed(ActionEvent event) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				TraderTo traderTo=TradersDaofactory.getTraderDao().search(objInvoiceNumberTextField.getText().trim());
				...
				objLabelTotalAmountLabel.setText(traderTo.getTotal().toString());
				for(MeterialDescriptionTO meterialDescriptionTO:traderTo.getobjMeterialDescriptionTo()){
					String[] data={meterialDescriptionTO.getMeterialDescription(),meterialDescriptionTO.getQuantity(),meterialDescriptionTO.getUnit(),meterialDescriptionTO.getAmount().toString()};
					objDefaultTableModel.addRow(data);
				}
			}
		});
	...
	private void objSaveButtonActionActionPerformed(ActionEvent event) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				traderTo.setVehicleNumber(objVechicleNoTextField.getText().trim().toString());
				traderTo.setPhoneNumber(objPhoneNumberTextFields.getText().trim().toString());
				...
				//
				traderTo.setPartyTINNo(objTINnoTextField.getText().trim().toString());
				traderTo.setTax(Double.parseDouble(objTaxTextField.getText().trim()));
				traderTo.setTotalAmount(Double.parseDouble(objTotalLabel.getText().trim()));
				traderTo.setTotal(Double.parseDouble(objLabelTotalAmountLabel.getText().trim()));
				for(int i=0;i<objDefaultTableModel.getRowCount();i++){
					MeterialDescriptionTO meterialDescriptionTO=new MeterialDescriptionTO();
					meterialDescriptionTO.setMeterialDescription(objMeterialdetailsTable.getModel().getValueAt(i,0).toString());
					meterialDescriptionTO.setQuantity(objMeterialdetailsTable.getModel().getValueAt(i,1).toString());
					meterialDescriptionTO.setUnit(objMeterialdetailsTable.getModel().getValueAt(i,2).toString());
					meterialDescriptionTO.setAmount(Double.parseDouble(objMeterialdetailsTable.getModel().getValueAt(i,3).toString()));
...
//			value = formatter.format((Number)value);
			...
			Double total=0.0;
			for(int i=0;i<objDefaultTableModel.getRowCount();i++){
				total=total+Double.parseDouble(objMeterialdetailsTable.getModel().getValueAt(i,3).toString());
			}
